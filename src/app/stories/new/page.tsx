...
export default function NewStoryPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (ideaSummary.trim().length < 10 || ideaSummary.length > 500) {
      setError("Die Zusammenfassung muss zwischen 10 und 500 Zeichen lang sein.");
      setLoading(false);
      return;
    }
    if (whatWentWell.trim().length < 10 || whatWentWell.length > 5000) {
      setError("'Was lief gut?' muss zwischen 10 und 5.000 Zeichen lang sein.");
      setLoading(false);
      return;
    }
    if (whatWentWrong.trim().length < 10 || whatWentWrong.length > 5000) {
      setError("'Was lief schlecht?' muss zwischen 10 und 5.000 Zeichen lang sein.");
      setLoading(false);
      return;
    }
    if (lessonsLearned.trim().length < 10 || lessonsLearned.length > 5000) {
      setError("'Lessons Learned' muss zwischen 10 und 5.000 Zeichen lang sein.");
      setLoading(false);
      return;
    }
    ...
    if (storyError) {
      setError("Beim Erstellen der Story ist ein Fehler aufgetreten. Bitte versuche es erneut.");
      setLoading(false);
      return;
    }
  ...
  return (
    <div className="mx-auto max-w-3xl px-4 py-8 sm:px-6">
      ...
      <div className="flex items-center gap-3 mb-8">
        ...
        <div>
          <h1 className="text-2xl font-bold">Story teilen</h1>
          <p className="text-sm text-muted-foreground">
            Teile deine Gründer-Erfahrung mit der Community
          </p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        {/* Story Type Toggle */}
        <div>
          <label className="block text-sm font-medium mb-2">Art der Story</label>
        ...
        <div>
          <label className="block text-sm font-medium mb-1">
            Was war die Idee / das Projekt? <span className="text-red-500">*</span>
          </label>
          <textarea
            value={ideaSummary}
        ...
        <div className="flex gap-3 pt-4 border-t border-border">
          ...
          >
            {loading ? "Wird veröffentlicht..." : "Story veröffentlichen"}
          </button>
        </div>
      </form>
    </div>
