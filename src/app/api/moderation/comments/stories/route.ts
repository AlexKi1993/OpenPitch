...
export async function POST(request: NextRequest) {
  ...
  if (!story_id || !content || typeof content !== "string") {
    return NextResponse.json({ error: "story_id and content (string) are required" }, { status: 400 });
  }

  if (!isValidUUID(story_id)) {
  ...
  if (error) {
    console.error("Moderation story comment create error:", error.message);
